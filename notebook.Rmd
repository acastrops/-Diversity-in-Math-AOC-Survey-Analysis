...
```{r}
q6 <- ggplot(data, aes(Q1,fill = factor(Q6))) 
q6 + geom_bar(position="fill") + coord_flip()
...
```{r}
q7 <- ggplot(data, aes(Q1,fill = factor(Q7))) 
q7 + geom_bar(position="fill") + coord_flip()
```
...
q8 <- ggplot(data, aes(Q1,fill = factor(Q8))) 
q8 + geom_bar(position="fill") + coord_flip()
```
...
q9 <- ggplot(data, aes(Q1,fill = factor(Q9))) 
q9 + geom_bar(position="fill") + coord_flip()
```

### Question 10: "With practice it is easy for me to improve my skills in math."
```{r}
levels(data$Q10)
data$Q10 <-  factor(data$Q10,levels(data$Q10)[c(5,2,3,1,4)])


q10 <- ggplot(data, aes(Q1,fill = factor(Q10))) 
q10 + geom_bar(position="fill") + coord_flip()
```

### Question 11: "Men are generally better suited to mathematics than women."
```{r}
levels(data$Q11)
data$Q11 <-  factor(data$Q11,levels(data$Q11)[c(5,2,3,1,4)])


q11 <- ggplot(data, aes(Q1,fill = factor(Q11))) 
q11 + geom_bar(position="fill") + coord_flip()
```
### Question 12: "Many people believe that men are better suited to mathematics than women."
```{r}
levels(data$Q12)
data$Q12 <-  factor(data$Q12,levels(data$Q12)[c(5,2,3,1,4)])


q12 <- ggplot(data, aes(Q1,fill = factor(Q12))) 
q12 + geom_bar(position="fill") + coord_flip()
```
### Question 13: "Some racial ethnic groups are generally better suited to mathematics than others."

```{r}
# include plot where everything is grouped by ethnicity instead of gender
```
