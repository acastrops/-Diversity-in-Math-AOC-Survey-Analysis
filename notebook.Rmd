...
# Ethnicity
is_non_hispanic_white <- as.numeric(data$`Non-Hispanic White`)
is_minority <- as.numeric(!is_non_hispanic_white)
is_asian <- is.numeric(data$Asian)
...
+ Call: what type of model we ran, what options we specified, etc.
...
Calculating and store p-values:
...
These coefficients are called proportional odds ratios and we would interpret these pretty much as we would odds ratios from a binary logistic regression. For `gender`, we would say that for a one unit increase in `gender`, i.e., going from 0 (Not male) to 1 (Male), the odds of "Much more anxiety" applying versus "A little more anxiety" or "Much less anxiety" applying combined are 3.02 greater, given that all of the other variables in the model are held constant. Likewise, the odds "Much more anxiety" or "A little more anxiety" applying versus "Much less anxiety" applying is 3.01 times greater, given that all of the other variables in the model are held constant. 
...
get_olr <- function(response, df){
  ...
  summary(model)
  summary(update(anxiety_model, method = "logistic", Hess = TRUE), digits = 3)
  model2 <- stepAIC(model, ~.^2)
  anova(model, model2)
  return (model2)
}
```
...
# Plan is to to the same for following response variables:
