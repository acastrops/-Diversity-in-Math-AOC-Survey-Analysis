...
```{r order_factors}
# reorders factors to be in the following order: "First Year", "Second Year", "Third Year", "Fourth Year", "Fifth or more"
data$Q3 <-  factor(data$Q3,levels(data$Q3)[c(2,4,5,3,1)], ordered = TRUE)

# reorders factors to be in the following order: "Yes", "No" 
# data$Q6 <-  factor(data$Q6,levels(data$Q6)[c(2,1)], ordered = TRUE)

# reorders factors to be in the following order: "No", "Maybe", "Yes" 
data$Q7 <-  factor(data$Q7,levels(data$Q7)[c(2,1,3)], ordered = TRUE)

# redorders factors for all the questions with responses from "Strongly disagree" to "Strongly agree" to be in the folling order:
...
# redorders factors for all the questions with responses from "Well below average" to "Well above average" to be in the following order:
...
# reorders factors to be in the following order: "Very unlikely", "Somewhat unlikely", "Neutral", "Somewhat likely", "Very likely"
data$Q20 <- factor(data$Q20,levels(data$Q20)[c(5,3,1,2,4)], ordered = TRUE)

# reorders factors to be in the following order:  "Much more anxiety", "A little more anxiety", "About the same", "A little less anxiety", "Much less anxiety"  
data$Q22 <- factor(data$Q22,levels(data$Q22)[c(5,2,3,1,4)], ordered = TRUE)

# reorders factors to be in the following order: "Often", "Sometimes", "Rarely", "Never"
data$Q23 <- factor(data$Q23,levels(data$Q23)[c(2,4,3,1)], ordered = TRUE)

# reorders factors to be in the following order: "Very out of place", "Somewhat out of place", "Neutral", "Somewhat comfortable", "Very comfortable" 
data$Q24 <- factor(data$Q24,levels(data$Q24)[c(5,3,1,2,4)], ordered = TRUE)


# reorders factors to be in the following order: "No", "Yes, but not at New College", "Yes, at New College and possibly elsewhere"
# for both Q25 and Q26
...
A contingency table is a table showing the distribution of one variable in rows and another in columns, used to study the association between the two variables. It is also known as a cross-tabulation or "crosstab". To test whether there are significant differences between the groups, we will perform a chi-squared test of association.

Here, we will look at responses to question 16, *"How would you rate your mathematical ability, compared to the student population at New College?"*. Below is the contingency table for this question's responses:

```{r crosstab ability vs NCF, eval=TRUE, echo=TRUE}
gender_vs_new_college_perception <- table(data$Q1, data$Q16)
gender_vs_new_college_perception
```

An assumption of the chi-squared test is that each cell in the crosstab has an expected value of at least 5. Since we can see *observed* frequencies below that threshold, we should double check that our expected frequencies are also above five.

```{r expected value ability vs NCF}

# get per-level % of total for math ability question
frequencies_q16 <- data %>%
  group_by(Q16) %>%
  summarise(count = n()) %>%
  mutate(freq = count / sum(count)) %>%
  dplyr::select(freq) # Why is MASS shadowing dplyr functions here

frequencies_q16 <- matrix(frequencies_q16$freq)

# get per-level % of total for gender
frequencies_q1 <- data %>%
  group_by(Q1) %>%
  summarise(count = n()) %>%
  mutate(freq = count / sum(count)) %>%
...
frequencies_q1 <- matrix(frequencies_q1$freq)

# create a matrix of gender frequency * math ability and multiply everything by n
expected_freq <- (frequencies_q1 %*% t(frequencies_q16)) * nrow(data)
round(expected_freq, 2)
```

Our expected frequencies for males rating themselves well-below or well-above average are both under 5, so we need to consolidate the five response groups into three. In the chunk below we create the two functions we will be using:
...
And below is our actual test:

```{r crosstab_NCF_student_3groups, eval=TRUE, echo=TRUE}
gender_vs_NCF_student_perception <- table(data$Q1, to_three_averages(data$Q16))
gender_vs_NCF_student_perception
gender_vs_NCF_student_perception_test <- chisq.test(gender_vs_NCF_student_perception)
#mantelhaen.test(gender_vs_social_group_perception)
```


When performing a chi-squared test of independence, we find a significant result. We can expect male-identifying students to respond differently, although we cannot make inferences about whether males are more or less confident with a chi-squared test.

Additionally, we can perform this same analysis on question 22 ("*Compared to other subjects, do you feel more or less anxiety in mathematics classes?*") in advance of the more thorough logistic regression analysis that will be done later in the report:

```{r crosstab anxiety, eval=TRUE, echo=TRUE}
gender_vs_anxiety <- table(data$Q1, data$Q22)
gender_vs_anxiety
gender_vs_anxiety_test <- chisq.test(gender_vs_anxiety)
```

(Note that in this case, there is no need to reduce to three groups, since we have enough responses for each entry, by each gender)

In this case we also find a significant result. Male-identifying respondents respond differently from those who do not identify as male.

```{r crosstab stereotype, eval=TRUE, echo=TRUE}
gender_vs_stereotype <- table(data$Q1, data$Q23)
gender_vs_stereotype
gender_vs_stereotype_test <- chisq.test(gender_vs_stereotype)
```
...
**Possible answers:** "Much more anxiety", "A little more anxiety", "About the same", "A little less anxiety", or "Much less anxiety"  
...
```{r q22_mann_whitney, eval=TRUE, echo=TRUE}
q22_mw_results <- get_mann_whitney(data$Q1, data$Q22, data)
q22_mw_results
```

With a p-value of $2.313 \times 10^{-6}$ we **reject** the null hypothesis and conclude that there is a statistically significant difference between the responses of male students and non-male students when asked about the level of anxiety felt in math classes compared to other subjects.  

\  

***

**Question 16:** *How would you rate your mathematical ability compared to the student population at New College?* 

**Possible answers:** "Well below average", "Below average", "Average", "Above average", or "Well above average"
...
```{r q16_mann_whitney, eval=TRUE, echo=TRUE}
q16_mw_results <- get_mann_whitney(data$Q1, data$Q16, data)
q16_mw_results
```

With a p-value of $2.278 \times 10^{-6}$ we **reject** the null hypothesis and conclude that there is a statistically significant difference between the responses of male students and non-male students when asked about how they would rate their mathematical abilities compared to the student population at New College.
...
```{r save_workspace_for_presentation, echo=FALSE}
save.image("DiversityInMath.Rdata")
```
