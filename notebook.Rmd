...
As said above, the nature of the test does not include a p-value by default. However, one way to calculate it in this case is by comparing the t-value against the standard normal distribution, like a z-test. Of course this is only true with infinite degrees of freedom, but is reasonably approximated by large samples, becoming increasingly biased as sample size decreases.
...
For `is_stem`, we would say that for a one unit increase in `is_stem`, we would expect a `0.73` increase in the expected value of apply in the log odds scale, given that all of the other variables in the model are held constant.
...
For `gender`, we would say that for a one unit increase in `gender`, i.e., going from `0` (Not male) to `1` (Male), the odds of *"Much more anxiety"* applying versus *"A little more anxiety"* or *"Much less anxiety"* applying combined are `3.02` greater, given that all of the other variables in the model are held constant. 

Likewise, the odds *"Much more anxiety"* or *"A little more anxiety"* applying versus *"Much less anxiety"* applying is `3.02` times greater, with all things constant. 

\  

Now to automate everything:
...
get_olr <- function(response, df){
  ...
  exp <- exp(cbind(OR = coef(model), olr_ci))
  return (list(paste("Summary"), summary, paste("Confidence intervals:"),olr_ci, paste("Exponentiated estimates, OR, and CI:"), exp))
}
```
...
The confidence intervals for `gender`, `year`, and `is_stem` do not include 0; `is_minority` does: **There is evidence to support that a student's gender, year, and AOC are statistically significant predictors of their reported confidence levels.**
...
For `gender`, we would say that for a one unit increase in `gender`, i.e., going from `0` (Not male) to `1` (Male), the odds of *"Well above average"* applying versus *"Above average"* or *"Well below average"* applying combined are `3.15` greater, given that all of the other variables in the model are held constant. Likewise, the odds *"Well above average"* or *"Above average"* applying versus *"Well below average"* applying is `3.15` times greater, with all things constant. 

\  

#### Gender Discrimination

```{r olr_gender_discrimination}
...
The confidence intervals for `gender`, and `year` do not include 0; `is_minority` and `is_stem` do: **There is evidence to support that a student's gender and year are statistically significant predictors of their reported feelings of gender discrimination.**
...
```{r olr_racial_discrimination, eval=FALSE}
