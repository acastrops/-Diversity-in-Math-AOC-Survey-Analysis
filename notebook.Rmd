...
```{r data}
## Reading data in, 
data <- read.csv("~/Documents/StatInference1/Diversity-in-Math-AOC-Survey-Analysis/survey_responses.csv")
```

```{r}
# Reassigning column names, and creating dictionary for new column names
data = data[1:264,] #last 100 rows are blank
num_questions <- ncol(data)
questions <- data.frame(colnames(data))
