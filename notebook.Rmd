...
mw_data <- data
colnames(mw_data[,1:27]) <- as.character(questions)
mw_data$Q6 <-  factor(mw_data$Q6,levels(mw_data$Q6)[c(2,1)], ordered = TRUE)
levels(mw_data$Q24)
mw_data$Q7 <-  factor(mw_data$Q7,levels(mw_data$Q7)[c(2,1,3)], ordered = TRUE)

agree_questions <- mw_data[,8:14]
agree_questions <- catcolwise( function(my_col) factor(my_col,levels(my_col)[c(5,2,3,1,4)], ordered = TRUE))(agree_questions)
mw_data[,8:14] <- agree_questions

average_questions <- mw_data[,15:19]
average_questions <- catcolwise( function(my_col) factor(my_col,levels(my_col)[c(5,3,2,1,4)], ordered = TRUE))(average_questions)
mw_data[,15:19] <- average_questions

mw_data$Q20 <- factor(mw_data$Q20,levels(mw_data$Q20)[c(5,3,1,2,4)], ordered = TRUE)

mw_data$Q22 <- factor(mw_data$Q22,levels(mw_data$Q22)[c(5,2,3,1,4)], ordered = TRUE)

mw_data$Q23 <- factor(mw_data$Q23,levels(mw_data$Q23)[c(2,4,3,1)], ordered = TRUE)

mw_data$Q24 <- factor(mw_data$Q24,levels(mw_data$Q24)[c(5,3,1,2,4)], ordered = TRUE)

get_man_whitney <- function(ind_col, dep_col, df) {
  mw_stat <- wilcox.test(as.integer(dep_col) ~ as.integer(ind_col), data = df)
  return(mw_stat)
} 

mw_results <- catcolwise(function(my_col)  get_man_whitney(mw_data$Q1,my_col,mw_data)[3])(mw_data[,3:24])

colnames(mw_results) <- as.character(questions[3:24,1])
t(unnest(mw_results))
```
...
wilcox.test(as.integer(Q20) ~ as.integer(Q1), mw_data)
...
plot_data$Q6 <-  factor(plot_data$Q6,levels(plot_data$Q6)[c(2,1)])

plot_data$Q7 <-  factor(plot_data$Q7,levels(plot_data$Q7)[c(3,1,2)])

agree_questions <- plot_data[,8:14]
agree_questions <- catcolwise( function(my_col) factor(my_col,levels(my_col)[c(5,2,3,1,4)]))(agree_questions)
plot_data[,8:14] <- agree_questions

average_questions <- plot_data[,15:19]
average_questions <- catcolwise( function(my_col) factor(my_col,levels(my_col)[c(5,3,2,1,4)]))(average_questions)
plot_data[,15:19] <- average_questions

plot_data$Q20 <- factor(plot_data$Q20,levels(plot_data$Q20)[c(5,3,1,2,4)])

plot_data$Q22 <- factor(plot_data$Q22,levels(plot_data$Q22)[c(5,2,3,1,4)])

plot_data$Q23 <- factor(plot_data$Q23,levels(plot_data$Q23)[c(2,4,3,1)])

plot_data$Q24 <- factor(plot_data$Q24,levels(plot_data$Q24)[c(5,3,1,2,4)])

plot_data$Q3 <-  factor(plot_data$Q3,levels(plot_data$Q3)[c(2,4,5,3,1)])

plot_data$Q25 <- mapvalues(plot_data$Q25, from = levels(plot_data$Q25), to = c("No","Yes, at NCF & elsewhere","Yes, but not at NCF"))

plot_data$Q26 <- mapvalues(plot_data$Q26, from = levels(plot_data$Q26), to = c("No","Yes, at NCF & elsewhere","Yes, but not at NCF"))
...
<!-- To be included: plot describing distribution of AOC's  -->

<!-- ## Question 5: Which math classes have you taken at New College? -->
<!-- To be included: plot describing distribution of math classes taken by the respondents -->
...
### Question 26: Have you ever felt that you were perceived or treated differently in a math class due to yourrace or ethnicity?
