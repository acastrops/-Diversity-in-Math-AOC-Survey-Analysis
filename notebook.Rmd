...
Plan: Include plots for each question that describes the distribution of responses, grouped by either gender of ethnicity where appropriate, and interpret each figure.  See Question 6 and 8 for an example of the plots and interpretations. 
...
ggplot(gender,aes(Gender, fill = Gender)) + geom_bar() + coord_flip()
```


## Question 2: What is your race/ethnicity?
```{r}
q2 <- ggplot(plot_data, aes(`Race/Ethnicity`, fill = `Race/Ethnicity`)) 
q2 + geom_bar() + coord_flip()
```

## Question 3: What is your year at New College?
```{r}
plot_q3 <- ggplot(plot_data, aes(x = Q3, fill = Q3))
plot_q3 + geom_bar() + xlab('Year') + guides(fill=guide_legend(title="What is your year?"))
```
...
q6_title <- "Ever consider AOC in Math?"
plot_by_gender(q6_data,q6_title)
plot_by_ethnicity(q6_data,q6_title)
```
This plot shows that a larger percentage of the male respondents have considered an AOC in mathematics than the non-male respondents.

## Question 7: Are you interested in graduate school, in any discipline?
```{r}
q7_data <- plot_data$Q7
q7_title <- "Interested in grad school?"
...
q8_title <- "I find math interesting."
plot_by_gender(q8_data,q8_title)
plot_by_ethnicity(q8_data,q8_title)
```
While the majority of both male and non-male groups find math interesting, that majority is larger in the group of male respondents.  
...
q10_title <- "It is easy to improve my\nmath skills with practice"
